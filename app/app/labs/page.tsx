...
export default function GamingLabsPage(){
  ...
  useEffect(()=>{
    ...
    return()=>{active=false;subscription.unsubscribe();window.removeEventListener('focus',onFocus);window.removeEventListener('pageshow',onFocus)}
  },[])
  ...
  async function runBudget(){setBusy('budget');setMessage('');setOptions([]);try{const result=await optimizeBudgetServer(game,budget);setOptions(result as BudgetOption[]);if(!result.length)setMessage('Nenhuma configuração cabe nesse orçamento para a modalidade selecionada.')}catch(e){setMessage(explainError(e))}finally{setBusy(null)}}
  ...
  return <main className={styles.page}>
    <header className={styles.header}>
      <div><small>LOTO<span>SMART</span> • GAMING LABS</small><h1>Motores server-side</h1><p>Budget, Wheeling e Monte Carlo executam fora do navegador, com validação de plano, rate limit e auditoria no backend.</p></div>
    ...
    {tool==='budget'&&<section className={styles.panel}>
      ...
      {budgetGames.length>0&&<div className={styles.result}><b>Portfólio materializado: {budgetGames.length} jogos</b><p>A opção foi selecionada pelo Gaming Engine e transformada em combinações para análise. Use o Monte Carlo para avaliar a dispersão experimental dessa carteira.</p><button className={styles.secondary} onClick={()=>chooseTool('montecarlo')}>Testar esta carteira no Monte Carlo</button></div>}
    </section>}
    ...
    {tool==='montecarlo'&&<section className={styles.panel}>
      ...
      <button className={styles.primary} onClick={runMonteCarlo} disabled={Boolean(busy)}>{busy==='mc'?'Simulando 12.000 cenários…':'Rodar Monte Carlo no servidor'}</button>
