...
function readLabsContext(userId:string):LabsContext|null{
  try{
    const raw=sessionStorage.getItem(LABS_CONTEXT)
    if(!raw)return null
    const context=JSON.parse(raw) as LabsContext
    if(context.version!==1||context.userId!==userId||Date.now()-context.createdAt>24*60*60*1000||!Array.isArray(context.games)||!context.games.length)return null
    return context
  }catch{return null}
}
...
export default function GamingLabsPage(){
  ...
  const [context,setContext]=useState<LabsContext|null>(null)
  ...
  useEffect(()=>{
    ...
    async function load(){
      ...
      if(!active)return
      setSession(next)
      if(!next){setEnt(null);setContext(null);setReady(true);return}
      const savedContext=readLabsContext(next.user.id)
      if(savedContext){setContext(savedContext);setGame(savedContext.game);setWheelBase(Math.min(LOTTERIES[savedContext.game].universe,LOTTERIES[savedContext.game].minPick+2))}
    ...
    const {data:{subscription}}=supabase.auth.onAuthStateChange((_event,next)=>{
      ...
      if(!next){setEnt(null);setContext(null);setReady(true);return}
      const savedContext=readLabsContext(next.user.id)
      if(savedContext){setContext(savedContext);setGame(savedContext.game)}
      const cached=readCachedEnt(next.user.id)
  ...
  function resetResults(){setOptions([]);setBudgetChoice(null);setBudgetGames([]);setWheel(null);setMc(null);setMessage('');setBusy(null)}

  function changeGame(next:GameId){
    resetResults();setGame(next);setWheelBase(Math.min(LOTTERIES[next].universe,LOTTERIES[next].minPick+2))
    if(context&&context.game!==next){setContext(null);try{sessionStorage.removeItem(LABS_CONTEXT)}catch{}}
  }

  function chooseTool(next:Tool){setTool(next);setMessage('');const url=new URL(window.location.href);url.searchParams.set('tool',next);window.history.replaceState({},'',url)}

  function explainError(error:unknown){
    const text=error instanceof Error?error.message:String(error)
    if(text.includes('PLAN_REQUIRED'))return'Sua conta não possui acesso pago ativo para este recurso.'
    if(text.includes('UNAUTHORIZED'))return'Sua sessão expirou. Volte ao Portfólio e entre novamente.'
  ...
  async function runMonteCarlo(){
    const games=budgetGames.length?budgetGames:(context?.game===game?context.games:[])
    if(!games.length){setMessage('Monte Carlo precisa de uma carteira real. Gere jogos no Portfólio ou monte uma carteira no Budget Optimizer.');return}
    setBusy('mc');setMessage('');setMc(null)
  ...
  if(!session)return <main className={styles.center}><div><h1>Gaming Labs</h1><p>Entre no LotoSmart para acessar os motores server-side.</p><a href="/app">Voltar ao Portfólio</a></div></main>
  ...
  return <main className={styles.page}>
    <header className={styles.header}>
      <div><small>LOTO<span>SMART</span> • GAMING LABS</small><h1>Motores server-side</h1><p>Analise estratégias e volte ao Portfólio quando quiser gerar e salvar novos jogos.</p></div>
      <div className={styles.headerActions}><span>{ent.role==='admin'?'ADMIN':ent.plan.toUpperCase()}</span><a href="/app">Voltar ao Portfólio</a></div>
    </header>

    {context&&context.game===game&&<div className={styles.result}><b>Carteira carregada: {context.games.length} jogos</b><p>{context.source==='saved'?'Portfólio salvo em Meus Jogos':'Jogos recém-gerados no Portfólio'} • concurso {context.contest} • {context.picks} dezenas por jogo.</p></div>}
    ...
    {tool==='budget'&&<section className={styles.panel}>
      ...
      {budgetGames.length>0&&<div className={styles.result}><b>Carteira do Budget: {budgetGames.length} jogos</b><p>A opção selecionada foi materializada em combinações para análise.</p><div style={{display:'flex',gap:10,flexWrap:'wrap'}}><button className={styles.secondary} onClick={()=>chooseTool('montecarlo')}>Testar no Monte Carlo</button><a className={styles.secondary} href="/app">Gerar novos jogos no Portfólio</a></div></div>}
    </section>}
