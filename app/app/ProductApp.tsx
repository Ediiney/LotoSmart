...
export default function ProductApp(){
  ...
  useEffect(()=>{void loadPublic();const timer=window.setInterval(loadPublic,60000);return()=>window.clearInterval(timer)},[])
  ...
  useEffect(()=>{if(session){void loadMine();void loadNotifications()}else{setMy([]);setPrefs(null)}},[session])
  ...
  function persistLabsContext(context:LabsContext){
    try{sessionStorage.setItem(LABS_CONTEXT,JSON.stringify(context))}catch{}
  }
  ...
  function analyzeSaved(p:Portfolio){
    if(!session||!p.tickets?.length)return
    persistLabsContext({version:1,userId:session.user.id,game:p.game,contest:p.contest_number,picks:p.picks_per_game,games:p.tickets.map(t=>t.numbers),source:'saved',portfolioId:p.id,createdAt:Date.now()})
    window.location.assign('/app/labs?tool=montecarlo')
  }
  ...
  function explainSaveError(text:string){
    ...
    if(text.includes('INVALID_TICKET')||text.includes('INVALID_PICKS')||text.includes('NUMBER_OUT_OF_RANGE')||text.includes('DUPLICATE_NUMBERS'))return'Os jogos não passaram pela validação do servidor. Gere uma nova carteira.'
    return text
  }
  ...
  async function loadNotifications(){
    if(!session)return
    const {data}=await supabase.from('notification_preferences').select('result_available,any_prize,email_enabled').eq('user_id',session.user.id).maybeSingle()
    setPrefs(data as NotificationPrefs|null)
  }
  ...
  if(!ready||!session)return <main><div className="loadingBrand">Loto<span>Smart</span></div></main>
  ...
  const awarded=my.reduce((s,p)=>s+(p.tickets?.filter(t=>!!t.latest_prize_tier).length||0),0)
  const planLabel=ent?.role==='admin'?'ADMIN':(ent?.plan||'none').toUpperCase()

  return <main>
    <header><div className="brand">Loto<span>Smart</span><small> V{PRODUCT_VERSION}</small></div><div className="headActions"><span className="planRuntimeBadge" data-plan={ent?.plan||'none'}>{planLabel}</span>{ent?.role==='admin'&&<a className="ghost" href="/admin">Admin</a>}<span className="email">{session.user.email}</span><button className="ghost" onClick={()=>supabase.auth.signOut()}>Sair</button></div></header>
