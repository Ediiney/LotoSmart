...
export default function ProductApp({initialSession,initialEntitlements}:{initialSession:Session;initialEntitlements:Entitlements}){
  ...
  return <main>
    <header><a className="brand" href="/app" aria-label="Voltar para a tela inicial" style={{textDecoration:'none',color:'inherit'}}>Loto<span>Smart</span><small> V{PRODUCT_VERSION}</small></a><div className="headActions"><span className="planRuntimeBadge" data-plan={ent.plan}>{planLabel}</span>{ent.role==='admin'&&<a className="ghost" href="/admin">Admin</a>}<span className="email">{session.user.email}</span><button className="ghost" onClick={()=>supabase.auth.signOut()}>Sair</button></div></header>
