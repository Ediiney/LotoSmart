...
function readCache(userId:string):Entitlements|null{
  try{
    const raw=localStorage.getItem(ACCESS_CACHE)
    if(!raw)return null
...
export default function PaidAccessGate(){
  ...
  async function refreshEntitlements(current:Session,background=false){
    ...
    if(!background)setEntLoading(true)
    try{
      const result=await withTimeout(supabase.rpc('get_my_entitlements'),ENTITLEMENT_TIMEOUT)
      if(!result.error&&result.data){const next=result.data as Entitlements;setEnt(next);writeCache(current.user.id,next);setMessage('')}
      else if(!background)setMessage('Não foi possível sincronizar seu plano agora. Tente novamente.')
    }catch{
      if(!background)setMessage('A validação do plano demorou mais que o esperado. Tente novamente.')
      window.setTimeout(()=>{if(current)void refreshEntitlements(current,true)},1200)
  ...
  useEffect(()=>{
    ...
    async function bootstrap(){
      ...
      if(!next){setEnt(null);return}
      const cached=readCache(next.user.id)
      if(cached){setEnt(cached);window.setTimeout(()=>void refreshEntitlements(next,true),0)}
      else window.setTimeout(()=>void refreshEntitlements(next),0)
    }
    void bootstrap()
    const {data:{subscription}}=supabase.auth.onAuthStateChange((_event,next)=>{
      if(!active)return
      setSession(next);setReady(true)
      if(!next){setEnt(null);writeCache('',null);return}
      const cached=readCache(next.user.id)
      if(cached)setEnt(cached)
      window.setTimeout(()=>void refreshEntitlements(next,!cached),0)
    })
    const onPageShow=()=>{if(document.visibilityState==='visible'&&session)window.setTimeout(()=>void refreshEntitlements(session,true),0)}
  ...
  async function authenticate(){
    ...
    try{
      ...
      if(mode==='login'){
        const {data,error}=await withTimeout(supabase.auth.signInWithPassword({email:clean,password}),8000)
        if(error){setMessage(error.message.toLowerCase().includes('invalid login')?'E-mail ou senha incorretos.':error.message);return}
        next=data.session
      }else{
        const {data,error}=await withTimeout(supabase.auth.signUp({email:clean,password,options:{emailRedirectTo:'https://lotosmart-ediineys-projects.vercel.app/app'}}),8000)
        if(error){setMessage(error.message);return}
        if(!data.session){setMessage('Conta criada. Confirme seu e-mail e depois entre para escolher um plano.');return}
        next=data.session
      }
      if(next){setSession(next);setReady(true);const cached=readCache(next.user.id);if(cached)setEnt(cached);window.setTimeout(()=>void refreshEntitlements(next,!cached),0)}
