...
export default function Home() {
  ...
  async function runStatisticalValidation(){
    ...
    try{
      ...
      if(error){setMsg(`Falha no Validation Engine: ${error.message || 'erro de comunicação com o backend'}`);return}
  ...
  if (!session) return <main className="publicPage">
    <header><div className="brand">Loto<span>Smart</span><small> V1.14.0</small></div><button className="login" onClick={() => setAuthOpen(true)}>Entrar / Criar conta</button></header>
  ...
  return <main>
    <header><div className="brand">Loto<span>Smart</span><small> V1.14.0</small></div><div className="headActions"><span className="email">{session.user.email}</span><button className="ghost" onClick={() => supabase.auth.signOut()}>Sair</button></div></header>
    ...
    {view==='validation'&&<section className="panel full validationPanel"><div className="outputHead validationHead"><div><p className="eyebrow">STATISTICAL VALIDATION ENGINE V2</p><h2>Backtest, estabilidade e incerteza</h2><p className="note">Compara 4 jogos estruturais com carteiras aleatórias de mesmo custo em concursos históricos. Agora o motor também mede intervalo de confiança e estabilidade por janelas. Continua sendo auditoria histórica, não previsão.</p></div>{validation&&<div className={`statusPill evidence-${validation.evidence}`}><small>Evidência</small><strong>{validation.evidence==='positive'?'SINAL POSITIVO':validation.evidence==='negative'?'SINAL NEGATIVO':'INCONCLUSIVO'}</strong></div>}</div><div className="validationControls"><label>Concursos no teste <b>{validationSample}</b></label><input type="range" min="50" max="200" step="10" value={validationSample} onChange={e=>setValidationSample(Number(e.target.value))}/><button className="cta inlineCta" disabled={validationBusy} onClick={runStatisticalValidation}>{validationBusy?'Executando backtest…':'Rodar validação histórica'}</button></div>{validation?<><div className="validationCompare"><article><span>LotoSmart estrutural</span><strong>{validation.structural_target_rate}%</strong><small>concursos com ao menos {validation.target_hits} acertos em um dos 4 jogos</small></article><article><span>Baseline aleatória</span><strong>{validation.random_target_rate}%</strong><small>média de {validation.baseline_repetitions} carteiras aleatórias por concurso</small></article><article className={validation.difference_pp>0?'positiveDelta':validation.difference_pp<0?'negativeDelta':''}><span>Diferença histórica</span><strong>{validation.difference_pp>0?'+':''}{validation.difference_pp} p.p.</strong><small>IC95%: {validation.difference_ci95?.[0]} a {validation.difference_ci95?.[1]} p.p.</small></article></div><div className="validationGrid"><div><small>Vitórias vs mediana aleatória</small><strong>{validation.wins}</strong></div><div><small>Empates</small><strong>{validation.ties}</strong></div><div><small>Derrotas</small><strong>{validation.losses}</strong></div><div><small>Janelas positivas</small><strong>{validation.positive_windows}/{validation.total_windows}</strong></div></div><div className="validationGrid"><div><small>Melhor cartão médio • LotoSmart</small><strong>{validation.structural_best_hits_avg}</strong></div><div><small>Melhor cartão médio • aleatório</small><strong>{validation.random_best_hits_avg}</strong></div><div><small>Percentil estrutural médio</small><strong>{validation.structural_mean_percentile}%</strong></div><div><small>Amostra</small><strong>{validation.sample_size} concursos</strong></div></div><div className="stabilityGrid">{validation.window_differences?.map(w=><div key={w.label}><small>{w.label} • {w.from}–{w.to}</small><strong className={w.difference_pp>0?'okText':w.difference_pp<0?'negativeText':''}>{w.difference_pp>0?'+':''}{w.difference_pp} p.p.</strong></div>)}</div><div className="validationDisclaimer"><b>Como interpretar</b><p>{validation.interpretation} “Sinal positivo” só aparece quando o intervalo de 95% da diferença histórica fica acima de zero. “Inconclusivo” significa que a amostra não separa com segurança o motor estrutural da baseline. Nenhum dos dois implica maior probabilidade das dezenas futuras.</p></div></>:<div className="empty compact validationEmpty"><div>∑</div><p>Rode o backtest para comparar o motor estrutural com apostas aleatórias em até 200 concursos históricos.</p></div>}</section>}
