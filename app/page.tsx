...
export default function Home() {
  ...
  const [view, setView] = useState<'portfolio'|'budget'|'wheel'|'montecarlo'|'data'|'mine'|'metrics'|'alerts'>('portfolio')
  ...
  useEffect(() => { setPicks(LOTTERIES[game].minPick);setWheelBase(Math.min(LOTTERIES[game].universe,LOTTERIES[game].minPick+2)); setGenerated([]);setBudgetGames([]);setMc(null);setWheel(null) }, [game])
  useEffect(() => {
  ...
  if (!session) return <main className="publicPage">
    <header><div className="brand">Loto<span>Smart</span><small> V1.11.0</small></div><button className="login" onClick={() => setAuthOpen(true)}>Entrar / Criar conta</button></header>
  ...
  return <main>
    <header><div className="brand">Loto<span>Smart</span><small> V1.11.0</small></div><div className="headActions"><span className="email">{session.user.email}</span><button className="ghost" onClick={() => supabase.auth.signOut()}>Sair</button></div></header>
    ...
    <nav className="sections"><button onClick={() => setView('portfolio')} className={view === 'portfolio' ? 'active' : ''}>Portfólio</button><button onClick={() => setView('budget')} className={view === 'budget' ? 'active' : ''}>Orçamento</button><button onClick={() => setView('wheel')} className={view === 'wheel' ? 'active' : ''}>Wheeling Lab</button><button onClick={() => setView('montecarlo')} className={view === 'montecarlo' ? 'active' : ''}>Monte Carlo</button><button onClick={() => setView('data')} className={view === 'data' ? 'active' : ''}>Data Agent</button><button onClick={() => setView('mine')} className={view === 'mine' ? 'active' : ''}>Meus Jogos</button><button onClick={() => setView('metrics')} className={view === 'metrics' ? 'active' : ''}>LotoSmart em números</button><button onClick={() => setView('alerts')} className={view === 'alerts' ? 'active' : ''}>Alertas</button></nav>
    ...
    {view === 'mine' && <section className="panel full"><div className="outputHead"><div><p className="eyebrow">MINHA CONTA</p><h2>Meus Jogos</h2></div><div className="miniStats"><span>{totalSaved} salvos</span><span>{checked} conferidos</span><span>{awarded} premiados</span></div></div>{my.length === 0 ? <div className="empty"><p>Você ainda não salvou nenhum portfólio.</p></div> : my.map(p => <article className="savedCard" key={p.id}><div><b>{LOTTERIES[p.game].name} • concurso {p.contest_number}</b><small>{money.format(Number(p.total_cost))}</small></div><div className="savedTickets">{p.tickets?.map((t, i) => <div key={t.id}><span>Jogo {i + 1}</span><Balls nums={t.numbers} /><em>{t.status === 'checked' ? `${t.latest_hits ?? 0} acertos` : 'Aguardando resultado'}</em></div>)}</div></article>)}</section>}
    ...
    {view === 'alerts' && <section className="panel full"><div className="outputHead"><div><p className="eyebrow">NOTIFICAÇÕES</p><h2>Alertas inteligentes</h2></div><div className="metric"><small>WhatsApp</small><strong className={prefs?.whatsapp_enabled&&profile?.whatsapp_opt_in_at?'okText':'warnText'}>{prefs?.whatsapp_enabled&&profile?.whatsapp_opt_in_at?'ATIVO':'PENDENTE'}</strong></div></div><p className="note">O Checker Agent confere jogos salvos automaticamente. Os alertas só usam eventos reais persistidos no banco.</p><div className="alertGrid"><div><b>Resultado disponível</b><p>Notificar quando o concurso dos seus jogos for confirmado.</p><button className="ghost fullBtn" onClick={()=>prefs&&setAlertPreference('result_available',!prefs.result_available)}>{prefs?.result_available?'Ativado':'Desativado'}</button></div><div><b>Premiação detectada</b><p>Notificar quando um jogo salvo atingir uma faixa de prêmio.</p><button className="ghost fullBtn" onClick={()=>prefs&&setAlertPreference('any_prize',!prefs.any_prize)}>{prefs?.any_prize?'Ativado':'Desativado'}</button></div><div><b>WhatsApp oficial</b><p>{profile?.whatsapp_phone_e164?`Número cadastrado: ${profile.whatsapp_phone_e164}`:'Cadastre um número para receber alertas.'}</p><button className="ghost fullBtn" onClick={()=>prefs&&setAlertPreference('whatsapp_enabled',!prefs.whatsapp_enabled)}>{prefs?.whatsapp_enabled?'Alertas WhatsApp ativos':'Ativar WhatsApp'}</button></div></div><div className="budgetResult"><h3>Status da integração</h3><p className="note">Checker Agent e fila de mensagens estão ativos. O envio externo depende da conexão da conta Meta WhatsApp Business e dos templates aprovados.</p><button className="cta inlineCta" disabled={waBusy} onClick={dispatchWhatsApp}>{waBusy?'Processando…':'Testar fila do WhatsApp'}</button></div></section>}
