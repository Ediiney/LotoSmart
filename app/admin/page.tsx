...
export default function AdminPage(){
  ...
  async function loadAll(){
    ...
    setProfiles((p.data||[]) as Profile[]);setSubscriptions((s.data||[]) as Subscription[]);setTransactions((t.data||[]) as Transaction[]);setEntries((e.data||[]) as Entry[])
  }
  ...
  async function addEntry(e:FormEvent){
    ...
    setForm({...form,description:'',amount_brl:'',vendor:''});setMessage('Lançamento registrado.');await loadAll()
  }

  if(!ready)return <main className={styles.center}>Carregando painel...</main>
  if(!session)return <main className={styles.center}><div><h1>Acesso administrativo</h1><p>Entre no LotoSmart antes de abrir o painel.</p><a href="/">Voltar para login</a></div></main>
  if(!authorized)return <main className={styles.center}><div><h1>Acesso restrito</h1><p>Sua conta não possui permissão administrativa.</p><a href="/">Voltar ao LotoSmart</a></div></main>

  return <main className={styles.page}>
    <header><div><p>LOTO<span>SMART</span> • ADMIN</p><h1>Visão do negócio</h1></div><a href="/">Abrir produto</a></header>
    <section className={styles.metrics}>
      <Metric label="Usuários" value={String(profiles.length)}/><Metric label="Assinantes" value={String(metrics.paying)}/><Metric label="Founders" value={`${metrics.founders}/100`}/><Metric label="MRR estimado" value={brl.format(metrics.mrr)}/><Metric label="Receita líquida" value={brl.format(metrics.revenue)}/><Metric label="Custos" value={brl.format(metrics.expenses)}/><Metric label="Resultado" value={brl.format(metrics.profit)}/>
    </section>

    <div className={styles.twoCol}>
      <section className={styles.panel}><h2>Controle de caixa</h2><p>Registre custos de Vercel, Supabase, domínio, WhatsApp, IA, marketing e outras receitas/despesas.</p>
        <form className={styles.form} onSubmit={addEntry}>
          ...
          <button>Registrar lançamento</button>{message&&<small>{message}</small>}
        </form>
      </section>
      <section className={styles.panel}><h2>Últimos lançamentos</h2><div className={styles.rows}>{entries.slice(0,10).map(e=><div key={e.id}><span><b>{e.description}</b><small>{e.category} • {e.occurred_on}</small></span><strong className={e.entry_type==='expense'?styles.negative:styles.positive}>{e.entry_type==='expense'?'-':'+'}{brl.format(Number(e.amount_brl))}</strong></div>)}{!entries.length&&<p className={styles.empty}>Nenhum lançamento ainda.</p>}</div></section>
    </div>

    <section className={styles.panel}><h2>Usuários recentes</h2><div className={styles.table}><div className={styles.tableHead}><span>E-mail</span><span>Perfil</span><span>Plano</span><span>Cadastro</span></div>{profiles.slice(0,15).map(p=><div key={p.id}><span>{p.email||'—'}</span><span>{p.role}</span><span>{p.plan_code}</span><span>{new Date(p.created_at).toLocaleDateString('pt-BR')}</span></div>)}</div></section>
  </main>
}
